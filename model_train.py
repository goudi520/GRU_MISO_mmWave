...
def modelTrain(net,trn_loader,val_loader,test_loader,options_dict):
    """
    ...
    for epoch in range(options_dict['num_epochs']):
        ...
        batch_score = 0
        with torch.no_grad():
            for v_batch, beam in enumerate(test_loader):
                init_beams = beam[:, :options_dict['inp_seq']].type(torch.LongTensor)
                inp_beams = embed(init_beams)
                inp_beams = inp_beams.cuda()
                batch_size = beam.shape[0]
                ...
                targ = targ.view(batch_size, options_dict['out_seq'])
                targ = targ.cuda()
                h_test = net.initHidden(beam.shape[0]).cuda()
                out, h_test = net.forward(inp_beams, h_test)
                pred_beams = torch.argmax(out, dim=2)
                pred_beams = pred_beams.cpu().numpy()
                targ = targ.cpu().numpy()
                beam_np = beam.cpu().numpy()
                bea_pre_targ = np.hstack((beam_np, pred_beams, targ))
                sio.savemat((options_dict['duibi_file'] + 'bea_pre_targ_epoch_' + str(epoch) + 'in5out3_1.mat'),{'name': bea_pre_targ})
    ...
    #torch.save({'state_dict': net.state_dict(), }, 'D:/pycharm/VABT-master/VABT-master/128marry16marry-out2.pth.tar')
